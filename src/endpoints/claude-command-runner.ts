// Read-side access to a Claude worker's native transcript store. Turns run on a
// ClaudeHost (one long-lived SDK query per session); this seam only reads the JSONL
// Claude itself writes, which stays the durable conversation store and the only
// session-discovery source. Local Claude reads the files directly; remote Claude reads
// them over ssh.
