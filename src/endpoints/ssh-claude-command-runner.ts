// Read side of a remote Claude worker. Turns run inside `qiyan-claude-host` on the worker's
// machine (see ssh-claude-host.ts); this class only reads the native JSONL that Claude
// itself writes there — durable history and session discovery — and runs a worker's
// `monitor` shell check on ITS host. Every operation is a plain attested ssh command, so it
// needs no runtime generation of its own and survives the host being restarted underneath it.
...
export class SshClaudeCommandRunner implements ClaudeCommandRunner {
  ...
  // have been swapped between reads, so we prove its identity again (mirrors
