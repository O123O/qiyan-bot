...
export const ASSISTANT_TOOL_SCHEMAS = {
  ...
  disconnect_endpoint: z.object({
    endpoint: endpoint().min(1).default("local"),
    force: z.boolean().default(false).describe("stop without proving every managed thread idle; interrupts an in-flight turn"),
  }).strict(),
  restart_endpoint: z.object({
    endpoint: endpoint().min(1).default("local"),
    force: z.boolean().default(false).describe("restart without proving every managed thread idle; interrupts an in-flight turn"),
  }).strict(),
  recover_endpoint: z.object({ endpoint: endpoint().min(1).default("local") }).strict(),
...
export const TOOL_DESCRIPTIONS: Partial<Record<AssistantToolName, string>> = {
  ...
  unadopt_session: "Release a session WITHOUT archiving its native thread or deleting project files — it stays discoverable/re-adoptable. Works even when the endpoint is unreachable, reporting endpoint_unreachable when idleness could not be proven.",
  archive_session: "Release a session and mark its native thread archived (still returned by discover_sessions, flagged archived; unadopt leaves it unarchived). Requires a reachable endpoint, because archiving is a claim about the remote thread; if the host is down, unadopt now and re-adopt then archive once it answers.",
  ...
  disconnect_endpoint: "Disconnect an endpoint's runtime; managed sessions recover on next use. Pass force:true to stop an endpoint whose worker status cannot be read or is still working — it interrupts that turn.",
  restart_endpoint: "Restart an endpoint's runtime and restore its managed sessions. Pass force:true to restart an endpoint whose worker status cannot be read or is still working — it interrupts that turn, and is the way past a wedged or unknown status.",
  recover_endpoint: "Retry recovery for an endpoint's managed sessions WITHOUT stopping its runtime: interrupts no turn and needs no idle proof, so prefer it over restart_endpoint whenever the endpoint is reachable. Recovers sessions whose native state is unavailable or unknown, and may resume an idle worker's goal; it does NOT repair a session already reporting error, and no endpoint tool clears a stale writer lock on a remote host. Returns state: recovered | superseded (the endpoint changed under it — call again) | unreachable (go look at the host).",
