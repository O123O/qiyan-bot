...
test("reverse forwarding binds a REMOTE loopback port (bind-not-relax) on the ControlMaster", () => {
  const plan = planSshConnection("devbox", parseSshConfig(`${parsed}controlmaster auto\ncontrolpath /tmp/user-master\n`), "/private/runtime");
  const forward = buildSshReverseForwardArgs(plan, 34567, 40001);
  ...
  assert.ok(forward.includes("127.0.0.1:34567:127.0.0.1:40001"), "reverse forward spec");
  assert.ok(cancel.includes("127.0.0.1:34567:127.0.0.1:40001"));
  assert.deepEqual(forward.slice(forward.indexOf("-R"), forward.indexOf("-R") + 2), ["-R", "127.0.0.1:34567:127.0.0.1:40001"]);
  ...
  for (const bad of [[0, 40001], [65_536, 1], [1, -1]] as const) {
