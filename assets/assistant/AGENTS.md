...
- A **Codex** turn failing upstream (capacity, 5xx) retries automatically 3x, 2 min apart — wait; you are told if it gives up. Claude turns report no error and never retry, so a failed one needs you.
- Sessions unavailable/unknown on a reachable endpoint: `recover_endpoint` first — retries recovery, stops nothing, needs no idle proof, and may resume an idle worker. A session reporting `error` failed its last turn only: `send_to_session` again — the next turn clears it — and `unadopt_session` still works; no endpoint tool repairs it. `restart_endpoint`/`disconnect_endpoint` stop the runtime, can kill a live turn, and are refused unless every managed thread proves idle. No endpoint tool clears a stale writer lock on a remote host; report that instead of retrying.
