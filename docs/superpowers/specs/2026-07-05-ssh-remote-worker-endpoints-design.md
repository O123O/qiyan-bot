...
Before either the internal scheduler/assistant dispatcher or chat adapters accept new work on startup, QiYan first seeds conservative claims from durable state: every last-known remote `active_turn_id`, plus every unresolved `send_to_session` start whose pre-dispatch checkpoint identifies the endpoint, thread, mapping, and client-message correlation. This protects capacity even when an SSH endpoint remains unreachable. QiYan then reads every managed thread on successfully reconnected endpoints and replaces/reconciles those hints with authoritative nonterminal turns, including turns started outside the new process. Restored claims deduplicate by endpoint/thread/turn or operation identity and may conservatively exceed a newly lowered configured limit; they block new starts until full authoritative history releases enough capacity. An unavailable endpoint does not prevent QiYan from starting, but its conservative claims remain reserved. This cold-start reconstruction is separate from in-process connection-loss reconciliation.
