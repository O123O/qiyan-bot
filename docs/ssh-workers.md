...
QiYan can run ordinary project sessions on remote Linux machines while the assistant remains local. Remote Codex runs its App Server in a detached tmux session. Remote Claude runs `qiyan-claude-host` in a detached tmux session the same way, holding one long-lived Agent SDK session per managed thread behind an owner-only Unix socket. A dropped SSH connection or QiYan service restart detaches observation without aborting either provider's turn. Local remains the default endpoint.

## Remote requirements

Install Node.js 24 or newer and `tmux` on the remote host, plus Codex 0.144.4 or newer for a Codex endpoint, or both the `claude` CLI and `@anthropic-ai/claude-agent-sdk` (`npm i -g @anthropic-ai/claude-agent-sdk`, resolvable from the npm global root or the account's own `node_modules`) for a Claude endpoint. Authenticate the selected CLI on that host as the SSH user and configure it for non-interactive automatic work; chat approvals are unsupported. QiYan does not copy local authentication or configuration.
...
You can then ask QiYan to create or adopt a session on `devbox`, inspect its models, recover it, disconnect it, or restart it. `recover_endpoint`, `disconnect_endpoint`, and `restart_endpoint` default to local when no endpoint is supplied. Disconnect/restart stop the runtime, so they refuse active or unprovable managed threads — including any thread whose state reads `error` or `unknown`, which is why a stuck session can otherwise wedge every later restart of its endpoint. `recover_endpoint` retries recovery **without** stopping the runtime: it interrupts no turn, needs no idle proof, and is the first thing to try when sessions are unavailable but the endpoint is reachable. It does not repair a session reporting `error` — that state is the app-server's verdict on the session's last turn, not on the thread, and sending to the worker again clears it (the next turn is accepted and flips the thread back to active); `unadopt_session` and `archive_session` also work on it. No endpoint tool clears a stale thread writer lock on the remote host — that needs the lock file replaced there.
...
Remote helper commands and the App Server user-space proxy use the same endpoint ControlMaster. Each connection generation prefers a live configured user master and otherwise selects a private QiYan-owned master. Before a selected user-owned socket is used, QiYan revalidates its local identity; the actual helper or proxy command is the authoritative liveness check. The proxy connects to the App Server Unix socket as the authenticated login user, then rechecks the exact socket inode and runtime identity before declaring readiness. QiYan strips the private readiness preamble, carries the WebSocket directly over that command channel, and treats WebSocket or SSH closure as connection loss. QiYan establishes its own key-authenticated master with `ControlPersist=yes` and never exits it — not on transport close, connection close, or runtime shutdown — so reactivating an endpoint reattaches instead of reauthenticating. That master still lives in QiYan's service cgroup, so it does not survive a service restart; only a master started in its own unit does. If the configured user master is gone and QiYan's BatchMode fallback then fails to authenticate, it reports which master needs re-establishing and keeps retrying on the usual escalating schedule, so the endpoint reconnects on its own once you restore the master.
