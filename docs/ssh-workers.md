...
Remote helper commands and the App Server user-space proxy use the same endpoint ControlMaster. Each connection generation prefers a live configured user master and otherwise selects a private QiYan-owned master. Before a selected user-owned socket is used, QiYan revalidates its local identity; the actual helper or proxy command is the authoritative liveness check. The proxy connects to the App Server Unix socket as the authenticated login user, then rechecks the exact socket inode and runtime identity before declaring readiness. QiYan strips the private readiness preamble, carries the WebSocket directly over that command channel, and treats WebSocket or SSH closure as connection loss. QiYan establishes its own key-authenticated master with `ControlPersist=yes` and never exits it — not on transport close, connection close, or runtime shutdown — so reactivating an endpoint reattaches instead of reauthenticating. That master still lives in QiYan's service cgroup, so it does not survive a service restart; only a master started in its own unit does. If the configured user master is gone and QiYan's BatchMode fallback then fails to authenticate, it pauses that endpoint's automatic retries and reports which master needs re-establishing, rather than retrying a failure only you can fix.
